...
- セルフレビューで見つけた不具合の修正:
  ...
  - 破線の罫線 `┄` `┅` `┈` `┉` `╌` `╍` は Source Han Sans が両端の点を
    ...
    「cmap か `fwid` で到達できる普通の全角字のインクが 1000 を超えない」
    ことを検査する（欧文ドナーの 2 セル合字は両ファミリーで 1200 幅なので
    対象外。CID 番号で絞る最初の版は Source Han Sans の疎な CID 空間の
    せいで漢字の 6 割を見ていなかった）（`Ⅷ` や Bold の `孰` が 20% 伸びた 2 ラウンドの間、
    罫線 10 字の検査だけでは通っていた）。README のヒント付けグリフ数
    （1,300〜1,800）を実測のおよそ 2,050〜2,550 に直した

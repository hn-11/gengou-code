...
- **基準を英語圏のターミナルフォントに置き換えた**（v5.0.0）。v4.0.0 までは
  ...
  - **Sumi Moji が持つ文字はすべて 1 セル**（ギリシャ・キリル・罫線・矢印
    ...
    Source Han Sans のギリシャ（602〜795）が 1 桁余計に取ることはなく、
    ウェイトによって幅が変わることもない。斜体は Source Code Pro Italic に
    ギリシャ・キリルが無いので、この 2 文字体系だけ Source Han Sans の
    比例幅（602〜1005）が残る。どちらも東アジア文字幅は曖昧（A）で
...
- セルフレビューで見つけた不具合の修正:
  ...
  - **斜体面のギリシャ・キリルが 1 セルに揃っていなかった**。
    ...
    Sans の比例幅（602〜1005）が残り、`grid_step` はいちばん近い段に
  ...
  - **欧文ドナーの `ccmp` が接ぎ木を越えていなかった**。`ccmp` は
    ...
    アクセント。正体 58・斜体 46 字）を `graft_halfwidth` と同じ扱いで

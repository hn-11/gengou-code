...
- **幅の方針**: Gengou Code が持つ文字はすべて 1 セル（ギリシャ・キリル・
  ...
  不要になり廃止（Source Han Sans 自身の `hwid` は残る。→ v6 で `fwid` も
  `hwid` も廃止、`docs/handover.md` の 1）。Source Han Sans の比例幅の残り（半角カナ 500、Hangul 字母
...
- **`fwid` を効かせても 2 セル合字は 600 グリッドの幅のまま**（`=>` が
  ...
  （→ v6 で `fwid` / `hwid` ごと廃止したので解消。`docs/handover.md` の 1）
...
round 12 の変異体レビューは 14 体中 12 体が全門番を通り、うち 10 体を
塞いだ（いずれも見つけた変異体で再現確認）:

- **C3（積み重ねマーク）はラウンド 11 が「帯では無理」と諦めたもの**。
  正しい規則は帯ではなく等式だった——*マークが mark-to-mark で持つ
  アンカーは、同じマークが mark-to-base で持つアンカーのどれかと一致
  する*。意味が同じなのだから座標も同じ。全 62 面が例外なく満たし、
  mkmk アンカーを 150 下げた変異体は JP で 77/77、Light Italic で
  81/81 が落ちる（`check_mkmk_anchors`）。
- **W3（かなを自分の箱の中で左右反転）**。送り・箱・インク面積・cmap が
  すべて保たれるので全門番が通していた。かな・漢字 13,944 字を
  `glyph_shape`（大きさと位置を割り去った 5 数値）でドナーと突き合わせる
  `check_donor_shapes` を入れた。全 20 JP 面で最悪 2e-12、Term 面の
  100 単位ずらしは不可視、反転した「し」は 1.0349。**フォントの 8 割の
  アウトラインを誰も読んでいなかった**のがこれで埋まる。1 面あたり 13 秒。
- **G10 / W2（欧文のアウトライン破壊）**。Bold の `e` を Light の `e` に、
  `0` と `O` の字形を入れ替え。`check_donor_letters` が文字・数字 62 字を
  Source Code Pro のその重みと 2 通りで突き合わせる——インク箱の大きさ
  （実測 1.0u / 境界 3u、入れ替えは 47u）と `glyph_shape`（実測 0.037 /
  境界 0.08、別重みは 0.327）。2 つは別のものを見る（入れ替えは形として
  0.013 しかない）ので両方問う。箱の位置はセル合わせで動かすので対象外、
  演算子は Monaspace のものなので対象外。
- **V12（可変フォントの既定マスターの `o` を 0.8 倍）**。VF は元から SCP と
  12 重みで厳密比較していたが、プローブが 5 字だった。全文字・数字に
  広げて全 12 重みで落ちる。
- **F1（JP 面から漢字 356 字を削除）**。`check_family_cmap` は基準面自身を
  比較できず、兄弟は縮んだ基準を含むので通る。Source Han Sans の
  16,742 字を JP 面の床にした（全 40 面が充足）。
- **L1 / L2 / L3（合字）**: `->` の規則が `<-` のグリフを出す、`calt` の
  SinglePos で 61 個が 90 単位ずれる、61 個が 0.45 倍の点になる。
  それぞれ「2 つの合字が同じグリフにならない」（全 60 面で 0 件）、
  「オフセットは常に (0,0)」（実測 (0,0) のみ）、「宣言幅の 0.40・部品の
  高さの 0.45 は覆う」（実測の最悪 0.512 / 0.627）で捕まえる。

独立に走らせた最終バッテリー（`b2999928` に対して）の結果:
ラウンド 11 の変異体 17 体は**全数 CAUGHT（盲点ゼロ）**、C3 を含む。
ラウンド 9 の 42 体は盲点 6 体（既知 8 体から J12・L15 が解消、退行なし）、
ラウンド 10 の 71 体は盲点 15 体（既知 22 体から G9・G12・G14・G15・
J24・J45・J47 の 7 体が解消、退行なし）。G10 と V12 も最終状態では
CAUGHT。

塞げなかったもの（測定値つきで据え置き）:

- **W1（Nerd Fonts のアイコンを左右反転）**。アイコンの門番は箱・縦横比・
  セルを跨ぐかを見るが、反転はそのすべてを保つ。`glyph_shape` は
  アイコン画では使えない——輪郭が互いに打ち消し合うものが多く、42 個は
  符号付き面積がほぼ 0 でモーメントが 6 桁に発散する（そのため
  `glyph_shape` は面積が箱の 1% 未満なら「読めない」を返す）。正当な
  ずれが 1.54 まで出る一方、反転したセパレータは 0.33 で、帯が引けない。
  ラスタライズして恒等・左右反転・上下反転・180 度回転のどれが最も
  合うかを比べる方法なら分離できる（全 10,395 アイコンで「反転の方が
  合う」ものは 0 件と実測済み）が、面ごとに 1 万字のラスタライズが要る。
- **R1（`f f _` の文脈で `i` を点無し `ı` に置換）**。`check_run_identity` は
  1 字と 2 字しか投げず、この規則は 3 グリフ要る。
  `check_substitution_identity` は `i → ı` を名前で許している
  （アクセントが載る直前だから）。「置換の先読みに結合マークがあること」を
  要求する規則が対応するが未実装。
- **R2（`calt` の連鎖文脈 GPOS が空白の送りを +120 する）**。既定で効く
  位置指定規則のうち横の送りを変えるのは、GDEF がマークと呼ぶグリフに
  対するものだけ——これを構造として要求すれば落ちる。未実装。
- **V1（可変フォントの `a` を Bold 端でのみ 60 単位横にずらす）**は
  `check_glyph_placement` の ±120 単位の余裕に収まるが、上の可変フォント
  比較（箱の位置も見る）で落ちるようになった。

round 12 のレンダリング欠陥探索は**欠陥ゼロ**で終わった。ドナー忠実性を
総当たりで測っている: 欧文レイヤーは JP / Term の全 10 ウェイトで
`dist/latin` と 1,335 字すべて一致（送り・箱・輪郭ごとの署名）、CJK は
Source Han Sans と 16,020 字を比べて JP 53 字・Term 79 字の差のみ
（記録済みの分類）、全角 15,776 字は dx=dy=0、半角かなは一律 +50、
欧文静的面は SCP を各重みでインスタンス化して 1,329 字をラスタ比較、
斜体のギリシャ・キリル 234 字 × 5 重みは Source Sans 3 Italic と 0 件差。
PUA アイコン 10,395 個を恒等・左右反転・上下反転・180 度回転で比べて
「反転の方が合う」ものは 0 件、`vert` / `vrt2` は 389 / 384 で欠落 0、
半角かな 63 字は全 20 面で (dx +50, dy 0, 500→600)、Term の罫線 167 字は
左右のインク輪郭が JP と同一。

したがって**「実レンダリング欠陥ゼロの回が 2 回連続」という停止条件は
ラウンド 12 で満たされた**（ラウンド 11 もゼロ）。

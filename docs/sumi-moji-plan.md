...
- **NF 版のアイコンは正方セルに収めている（本家パッチは 600 x 856）**。
  ...
  （600 / 2048）では箱が 600 x 600 になり、縦長のアイコン 3,260 字
  （10,369 字中）が本家パッチより最大 1.43 倍小さい。font-patcher 独自の
...
- **結合文字が縦組みで次のセルに落ちる**。接ぎ木した結合文字 53 字
  ...
  する方法で置いている。横組みでは正しく前の字に載るが、縦組みでは
  「1 セル左」が「1 字ぶん下」を意味しないので、NFD の `ḱ`（`k` +
  U+0301）は `k` の次のセルにアクセントが落ちる。GPOS の mark-to-base で
  付けるのが本筋だが、Source Han Sans 自身もこの 53 字を mark にも base に
  も入れていない（ドナーの縦組みでも同じ位置に落ちる）ので、こちらで
  アンカーを新設することになる。要判断。
